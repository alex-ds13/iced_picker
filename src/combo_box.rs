    ...
    Renderer: text::Renderer,
{
    ...
    /// Sets the message that will be produced when the Enter key is pressed
    /// while the [`ComboBox`] is focused and no menu option is selected,
    /// carrying the current text of the input. Nothing is produced when the
    /// text is empty.
    pub fn on_submit(mut self, on_submit: impl Fn(String) -> Message + 'a) -> Self {
                ...
                }) = event
                {
                    ...
                    match (named_key, shift_modifier) {
                        (key::Named::Enter, _) => {
                            ...
                            {
                                shell.publish((on_submit)(state.value.clone()));
                                published_message_to_shell = true;
                            }
