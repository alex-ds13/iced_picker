    ...
    Theme: Catalog + iced::widget::button::Catalog,
{
    ...
    /// The function that produces a message when the submit button of the [`ColorPicker`].
    on_submit: Box<dyn Fn(Color) -> Message + 'a>,
    /// The position of the [`ColorPicker`].
    // position: Point,
    /// The style of the [`ColorPicker`].
    ...
    Theme: 'a + Catalog + iced::widget::button::Catalog + iced::widget::text::Catalog,
{
    ...
    fn update_hsv_color(
        ...
        let hsv_color: Hsv = self.color.into();
        ...
        match event {
            ...
            | Event::Touch(touch::Event::FingerPressed { .. }) => {
                if cursor.is_over(sat_value_bounds) {
                    state.color_bar_dragged = ColorBarDragged::SatValue;
                    state.focus = Focus::SatValue;
                }
                if cursor.is_over(hue_bounds) {
                    state.color_bar_dragged = ColorBarDragged::Hue;
                    state.focus = Focus::Hue;
                }
            }
    ...
    fn update_rgba_color(
        ...
        match event {
            ...
            | Event::Touch(touch::Event::FingerPressed { .. }) => {
                if cursor.is_over(red_bar_bounds) {
                    state.color_bar_dragged = ColorBarDragged::Red;
                    state.focus = Focus::Red;
                }
                if cursor.is_over(green_bar_bounds) {
                    state.color_bar_dragged = ColorBarDragged::Green;
                    state.focus = Focus::Green;
                }
                if cursor.is_over(blue_bar_bounds) {
                    state.color_bar_dragged = ColorBarDragged::Blue;
                    state.focus = Focus::Blue;
                }
                if cursor.is_over(alpha_bar_bounds) {
                    state.color_bar_dragged = ColorBarDragged::Alpha;
                    state.focus = Focus::Alpha;
                }
            }
            Event::Mouse(mouse::Event::ButtonReleased(mouse::Button::Left))
            | Event::Touch(touch::Event::FingerLifted { .. } | touch::Event::FingerLost { .. }) => {
    ...
    fn on_event_keyboard(&mut self, state: &mut OverlayState, event: &Event) -> event::Status {
        ...
        if let Event::Keyboard(keyboard::Event::KeyPressed { key, .. }) = event {
            ...
            if matches!(key, keyboard::Key::Named(keyboard::key::Named::Tab)) {
                ...
                state.sat_value_canvas_cache.clear();
                state.hue_canvas_cache.clear();
            } else {
                ...
                match state.focus {
                    Focus::SatValue => status = sat_value_handle(key, &mut self.color),
                    Focus::Hue => status = hue_handle(key, &mut self.color),
                    Focus::Red => status = rgba_bar_handle(key, &mut self.color.r),
                    Focus::Green => status = rgba_bar_handle(key, &mut self.color.g),
                    Focus::Blue => status = rgba_bar_handle(key, &mut self.color.b),
                    Focus::Alpha => status = rgba_bar_handle(key, &mut self.color.a),
                    _ => {}
                }
            }
    ...
    fn update_mouse(
        ...
        state.mouse_interaction = mouse_interaction
...
where
    Message: 'a + Clone,
    Theme: 'a + Catalog + iced::widget::button::Catalog + iced::widget::text::Catalog + container::Catalog,
{
    ...
    fn state(&self) -> tree::State {
        tree::State::new(OverlayState::new(
            self.color,
            &self.cancel_button,
            &self.submit_button,
        ))
    ...
    fn layout(&mut self, tree: &mut Tree, renderer: &Renderer, limits: &Limits) -> Node {
        ...
        println!(
            "limits: {:?}\nbounds: {:?}\nmax_width: {}\nmax_height: {}",
            limits, bounds, max_width, max_height
        );
        ...
        let divider = if bounds.width > bounds.height {
            Row::<(), Theme, Renderer>::new()
                ...
                .width(max_width)
                .height(max_height)
                .layout(&mut state.tree[2], renderer, &limits)
        } else {
        ...
        // ----------- Block 1 ----------------------
        let block1_node = block1_layout(state, renderer, block1_bounds);

        // ----------- Block 2 ----------------------
        let block2_node = block2_layout(self, state, renderer, block2_bounds);
    ...
    fn update(
        ...
        let state = tree.state.downcast_mut::<OverlayState>();
        if event::Status::Captured == self.on_event_keyboard(state, event) {
            state.sat_value_canvas_cache.clear();
            state.hue_canvas_cache.clear();
            shell.capture_event();
            shell.request_redraw();
        ...
        self.update_hsv_color(state, event, block1_layout, cursor, shell);
        // ----------- Block 1 end ------------------
        ...
        self.update_rgba_color(state, event, rgba_color_layout, cursor, shell);
        ...
        if !fake_messages.is_empty() {
            shell.publish((self.on_submit)(self.color));
        }
        // ----------- Block 2 end ------------------
        ...
        if !matches!(
            ...
            let previous_interaction = state.mouse_interaction;
            self.update_mouse(state, layout, cursor, &Rectangle::default(), renderer);
            if previous_interaction != state.mouse_interaction {
                state.sat_value_canvas_cache.clear();
                state.hue_canvas_cache.clear();
                shell.request_redraw();
            }
    ...
    fn mouse_interaction(
        ...
        let state = tree.state.downcast_ref::<OverlayState>();
    ...
    fn draw(
        ...
        let block1_layout = children
            .next()
            .expect("Graphics: Layout should have a 1. block layout");
        block1(renderer, self, state, block1_layout, cursor, &style_sheet);
...
fn block1_layout(state: &mut OverlayState, renderer: &Renderer, bounds: Rectangle) -> Node {
    ...
    let block1_node = Column::<(), Theme, Renderer>::new()
        ...
        .layout(&mut state.tree[2], renderer, &block1_limits);
    ...
    Theme: 'a + Catalog + iced::widget::button::Catalog + iced::widget::text::Catalog,
{
    ...
    let cancel_button =
        color_picker
            .cancel_button
            .layout(&mut state.tree[0], renderer, &cancel_limits);
    ...
    let mut hex_text_layout = Row::<Message, Theme, Renderer>::new()
        ...
        .layout(&mut state.tree[2], renderer, &hex_text_limits);
    ...
    let rgba_tree = if let Some(child_tree) = state.tree[2].children.get_mut(0) {
        ...
        state.tree[2].children.insert(0, child_tree);
        &mut state.tree[2].children[0]
    };
    ...
    let mut submit_button =
        color_picker
            .submit_button
            .layout(&mut state.tree[1], renderer, &submit_limits);
...
fn block1<Message, Theme>(
    renderer: &mut Renderer,
    color_picker: &ColorPicker<'_, Message, Theme>,
    state: &OverlayState,
    layout: Layout<'_>,
    cursor: Cursor,
...
fn hsv_color<Message, Theme>(
    renderer: &mut Renderer,
    color_picker: &ColorPicker<Message, Theme>,
    state: &OverlayState,
    layout: Layout<'_>,
    cursor: Cursor,
    style_sheet: &HashMap<StyleState, Style>,
...
{
    let mut hsv_color_children = layout.children();
    let hsv_color: Hsv = color_picker.color.into();

    let sat_value_layout = hsv_color_children
        .next()
        .expect("Graphics: Layout should have a sat/value layout");
    let mut sat_value_style_state = StyleState::Active;
    if state.focus == Focus::SatValue {
    ...
    let geometry =
        state
            ...
            .draw(renderer, sat_value_layout.bounds().size(), |frame| {
...
pub struct OverlayState {
    ...
    /// The buttons tree
    pub(crate) tree: Vec<Tree>,
}

impl OverlayState {
    /// Creates a new State with the given color.
    #[must_use]
    pub fn new<Message, Theme>(
        color: Color,
        cancel_button: &dyn Widget<Message, Theme, Renderer>,
        submit_button: &dyn Widget<Message, Theme, Renderer>,
    ) -> Self
    where
        Message: Clone,
        Theme: text::Catalog + container::Catalog + iced::widget::button::Catalog,
    {
        ...
        let child_tree = Tree::new(element.as_widget());
        // tree[2].children.insert(0, child_tree);
        tree[2] = child_tree;
...
pub fn color<'a, Message, Theme, F>(
    // show_picker: bool,
    color: Color,
